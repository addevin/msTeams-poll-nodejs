...
// Initialize lowdb
const adapter = new FileSync('db.json'); // Change the filename as needed
const db = low(adapter);
db.defaults({ polls: [] }).write();

// Token generated by Teams 
const sharedSecret = process.env.MS_AUTH_TOKEN; // e.g. "+ZaRRMC8+mpnfGaGsBOmkIFt98bttL5YQRq3p2tXgcE="
            ...
            if (msgHash === auth || sharedSecret === auth) {
                ...
                switch (true) {
                    ...
                    case (receivedText.indexOf("thumbnail-card") != -1):
                        ...
                        break;
                    case (receivedText.indexOf("new poll") != -1):
                            ...
                            const newPoll = {
                                ...
                                options: [],
                                votes: [], // Store user information along with votes
                            };
                    
                            activePollId = newPoll.id;
                    
                            db.get('polls').push(newPoll).write();
                    
                            responseMsg = JSON.stringify({
                                type: 'message',
                                text: `New poll created with question: "${pollQuestion}"`,
                            });
                    ...
                    case receivedText.includes('add option'): {
                        ...
                        const [optionText] = receivedText.replace('add option', '').trim().split(' ');
                    
                        const poll = db.get('polls').find({ id: activePollId }).value();
                    
                        if (poll && poll.id === activePollId) {
                            const newOption = { id: poll.options.length+1, text: optionText, votes: [] };
                            poll.options.push(newOption);
                            db.write();
                            responseMsg = JSON.stringify({
                                type: 'message',
                                text: `Option "${optionText}" added to the active poll.`,
                            });
                        } else {
                            responseMsg = JSON.stringify({
                                type: 'message',
                                text: `Poll with ID "${activePollId}" not found or not the active poll.`,
                            });
                        }
                    ...
                    case receivedText.includes('vote'): {
                        const [pollId, optionId, user] = receivedText.replace('vote', '').trim().split(' ');
                    
                        // Check if an active poll exists
                        if (!activePollId) {
                            responseMsg = JSON.stringify({
                                type: 'message',
                                text: 'No active poll. Create a new poll first.',
                            });
                            break;
                        }
                        ...
                        if (poll && poll.id === activePollId) {
                            ...
                            if (option) {
                                ...
                                option.votes.push({ user, timestamp: new Date() });
                                db.write();
                                responseMsg = JSON.stringify({
                                    type: 'message',
                                    text: `Vote recorded for option "${option.text}" by user "${user}".`,
                                });
                            } else {
                                responseMsg = JSON.stringify({
                                    type: 'message',
                                    text: `Option with ID "${optionId}" not found in the active poll.`,
                                });
                            }
                        } else {
                            responseMsg = JSON.stringify({
                                type: 'message',
                                text: `Poll with ID "${pollId}" not found or not the active poll.`,
                            });
                        }
